...
export const commandTools = [
  {
    ...
    execute: async (
      args: { command: string; args: string[]; options?: { timeout?: number; maxBuffer?: number } },
      workspace: string,
    ) => {
  ...
  },
  {
    ...
    description:
      'Execute a simple command with shell features (pipes, redirections) in the workspace',
    parameters: {
      type: 'object',
      properties: {
        command: {
    ...
    execute: async (
      args: { command: string; options?: { timeout?: number; maxBuffer?: number } },
      workspace: string,
    ) => {
